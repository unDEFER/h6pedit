...
void invert(Color *color, ErrCorrection correction)
{
    color_convert(color, &ITP_SPACE, correction);
    if (color.channels[0] < 0.5)
        color.channels[0] = 1.0 - 0.8*color.channels[0];
    else
        color.channels[0] = 0.8*(1.0 - color.channels[0]);
    color.channels[3] = 1.0;
}
...
void color_to_u8(Color *h6p, inout(ColorSpace) *rgbspace, ref ubyte[4] col, bool *err, ErrCorrection correction)
{
...
void color_convert(Color *h6p, inout(ColorSpace) *to_space, ErrCorrection correction)
{
    if (h6p.space is to_space) return;

    assert(!isNaN(h6p.channels[0]));

    if (h6p.space.bounds !is null) 
    {
