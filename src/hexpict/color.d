...
{
    auto ret = Pixel(cast(ushort) round(255.0 + 0.402*p.r - 1.174*p.g - 0.228*p.b),
            cast(ushort) round(255.0 - 0.598*p.r - 0.174*p.g - 0.228*p.b),
            cast(ushort) round(255.0 - 0.598*p.r - 1.174*p.g + 0.772*p.b),
            255);
    ...
    if (ma > 255)
    {
        ret.r = cast(ushort) round(ret.r*255.0/ma);
        ret.g = cast(ushort) round(ret.g*255.0/ma);
        ret.b = cast(ushort) round(ret.b*255.0/ma);
    }
